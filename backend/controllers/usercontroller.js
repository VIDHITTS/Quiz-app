...
async function getDashboard(req, res) {
  try {
    ...
    // Get recent attempts
    const recentAttempts = await Attempt.find({ student: userId })
      .populate('quiz', 'title')
      .select('quiz score totalQuestions percentage createdAt')
      .sort({ createdAt: -1 })
      .limit(5);
    ...
    res.json({
      ...
      dashboard: {
        stats: {
          totalQuizzes,
          totalAttempts,
          averageScore: avgScore.length > 0 ? Math.round(avgScore[0].avgPercentage) : 0
        },
